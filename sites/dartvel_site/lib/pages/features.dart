...
const List<(String, String, String)> shipped = <(String, String, String)>[
  ...
  (
    ...
    'worse than one that shows it unstyled. A grid can be told how many '
    'columns it has, too: the renderer read that and the exporter wrote '
    'it while the inspector offered no control, so a grid built here was '
    'permanently two columns wide. A layout property now says which '
    'layouts it means anything to, and is offered only there -- a row has '
    'no columns, and a control that does nothing is worse than one that is '
    'absent. With drag-and-drop, '
    ...
    'it, a row that wraps becomes one that wraps and a frame that crops '
    'its content crops it, a stroke on one side is a rule rather than a '
    'box drawn right round, an image is painted like every other layer so '
    'a round avatar arrives round, a layer pinned on top of a laid-out '
    'frame stays pinned rather than joining the row, a gradient that fades '
    'keeps its fade, images are '
