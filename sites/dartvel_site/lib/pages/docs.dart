...
/// Where each step of one docs page is, so its contents can reach it.
///
/// Held in a State so a GlobalKey stays the same object across rebuilds, and
/// one set per page rather than one for the program. The link preview builds a
/// live copy of the page, and a click navigates while that copy is still up,
/// so for a moment there are two pages. With one global set the second could
/// not take the keys and, in a release build, dropped every step: the page
/// showed its contents and then the footer.
///
...
class DocsSteps extends StatefulWidget {
  const DocsSteps({super.key, required this.builder});

  /// Builds the page with the keys its steps carry.
  ...
  static Map<String, GlobalKey>? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<_DocsStepKeys>()?.keys;

  @override
  State<DocsSteps> createState() => _DocsStepsState();
}

class _DocsStepsState extends State<DocsSteps> {
  final Map<String, GlobalKey> _keys = <String, GlobalKey>{
    for (final String id in kDocsOrder) id: GlobalKey(debugLabel: 'docs:$id'),
  };

  @override
  Widget build(BuildContext context) => _DocsStepKeys(
    keys: _keys,
    child: Builder(
...
class _DocsStepKeys extends InheritedWidget {
  const _DocsStepKeys({required this.keys, required super.child});

  final Map<String, GlobalKey> keys;

  @override
  bool updateShouldNotify(_DocsStepKeys oldWidget) => keys != oldWidget.keys;
}
...
void dvGoToStep(BuildContext context, String id) {
  final BuildContext? target = DocsSteps.maybeOf(context)?[id]?.currentContext;
...
@DVPage(
  title: 'Dartvel docs: install and build your first app',
  showAppBar: false,
)
@pragma('vm:entry-point')
Widget _docsPage(BuildContext context) => DocsSteps(
  builder: (BuildContext context, Map<String, GlobalKey> steps) =>
      SingleChildScrollView(
        child: DVBox.list(<Widget>[
          ...
          const DocsContents(),
          KeyedSubtree(key: steps['install'], child: const Install()),
          KeyedSubtree(key: steps['create'], child: const FirstApp()),
          KeyedSubtree(key: steps['pages'], child: const Pages()),
          KeyedSubtree(key: steps['models'], child: const Models()),
          KeyedSubtree(key: steps['backend'], child: const Backend()),
          KeyedSubtree(key: steps['links'], child: const Links()),
          KeyedSubtree(key: steps['state'], child: const Signals()),
          KeyedSubtree(key: steps['building'], child: const Building()),
          KeyedSubtree(key: steps['honesty'], child: const Honesty()),
          const SiteFooter(),
        ], spacing: 0),
      ),
);
...
Widget _docsContentsLine(
  ...
  return DVBox(
    ...
    DVBox.row(
      <Widget>[
        DVText(numbered ? '$number' : '•').modifier(
          const DVModifier()
              .fontSize(13)
...
Widget _firstApp() => const Section(
  children: <Widget>[
    ...
    CodeBlock(<String>['dartvel create my_app', 'cd my_app', 'dartvel dev']),
    Bullets(<String>[
