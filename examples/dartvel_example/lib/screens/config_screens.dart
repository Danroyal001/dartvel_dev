...
class ShopScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ShopScroll(children: children),
      );
}
...
class SettingsScreen extends StatelessWidget {
  ...
  static const List<String> tabs = <String>['general', 'delivery', 'notifications'];

  @override
  Widget build(BuildContext context) {
    final Palette p = Palette.of(context);
    final String current = tabs.contains(tab) ? tab! : 'general';
    return ShopScreen(title: 'Settings', children: <Widget>[
      DVBox.wrapLine([
      ...
      Material(
        ...
        child: DVBox.list([
        for (final (String title, String subtitle, bool on) in switch (current) {
          'delivery' => <(String, String, bool)>[
              ('Leave with a neighbour', 'If nobody is in', true),
              ('Letterbox-sized bags', 'Every 250 g bag fits through a door', true),
            ],
          'notifications' => <(String, String, bool)>[
              ('Roast day', 'When your order goes into the roaster', true),
              ('Shipping', 'When it leaves us, with tracking', true),
              ('New coffees', 'At most once a fortnight', false),
            ],
          _ => <(String, String, bool)>[
              ('Grind for me', 'Whole bean unless you choose a grind', false),
...
class TeamScreen extends StatelessWidget {
  ...
  static const Map<String, (String, String)> members = <String, (String, String)>{
  ...
  Widget build(BuildContext context) {
    final Palette p = Palette.of(context);
    return ShopScreen(title: 'The roasters', children: <Widget>[
      const DVText('Three people, one Probat roaster, and a small warehouse by '
              'the harbour.')
...
class TeamMemberScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ShopScreen(title: name, children: <Widget>[
      DVBox.row([
        DVBox(DVText(name.characters.first).modifier(p.display.color(p.onAccent)))
            .modifier(
          const DVModifier()
              .width(72)
              .height(72)
      ...
      ], spacing: 18, crossAlign: DVCrossAlign.center),
      DVText('Ask $name anything about the coffee: hello@oakline.coffee')
          .modifier(p.body),
    ]);
  }
}
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return ShopScreen(title: 'Checkout', children: <Widget>[
      WatchModels<Product>(
        watch: Product.watch,
        builder: (BuildContext context, List<Product>? catalog) {
          ...
          return DVBox.list([
            DVBox.list([
              ...
              const DVText('14 Quay Street, Bristol BS1 4DB').modifier(p.muted),
            ...
            DVBox.list([
              ...
              for (final CartLine line in cart.linesIn(catalog))
                SummaryLine('${line.quantity} × ${line.coffee.name}',
                    formatPrice(line.totalCents)),
              SummaryLine('Shipping', shipping == 0 ? 'Free' : formatPrice(shipping)),
              Divider(color: p.line),
              SummaryLine('Total', formatPrice(cart.totalCents(catalog)), strong: true),
              FilledButton(
                key: const Key('place-order'),
                onPressed: _placing ? null : () => _place(catalog),
                child: Text(_placing ? 'Placing order…' : 'Place order'),
              ),
              const DVText('This demo takes no payment.').modifier(p.muted.fontSize(13)),
...
class ReportsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Palette p = Palette.of(context);
    return ShopScreen(title: 'Manage the shop', children: <Widget>[
      ...
      WatchModels<Order>(
        ...
        builder: (BuildContext context, List<Order>? orders) {
          ...
          final int revenue =
              orders.fold(0, (int sum, Order o) => sum + o.totalCents);
          return DVBox.list([
            DVBox.wrapLine([
              CoffeeFact('Orders', '${orders.length}'),
              CoffeeFact('Revenue', formatPrice(revenue)),
              CoffeeFact('In progress',
                  '${orders.where((Order o) => o.status != 'delivered').length}'),
            ], spacing: 10),
            if (orders.isNotEmpty)
              DVBox(Order.Table(orders)).modifier(cardStyle(p, padding: 8)),
          ], spacing: 16);
        },
      ),
