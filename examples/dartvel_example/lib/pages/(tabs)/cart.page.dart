...
Widget _cartPage(BuildContext context) => (() {
      ...
      return WatchModels<Product>(
        ...
        builder: (BuildContext context, List<Product>? catalog) {
          ...
          if (lines.isEmpty) {
            return ShopScroll(children: <Widget>[
              ...
              EmptyState(
                ...
                message: 'Pick a coffee from this week’s shelf. We roast on '
          ...
          return ShopScroll(children: <Widget>[
            PageHeading('Your bag',
                subtitle: '${cart.count} ${cart.count == 1 ? 'bag' : 'bags'} of coffee'),
            DVBox.list([
              for (final CartLine line in lines)
                DVBox.row([
                  SizedBox(
                    width: 72,
                    child: BagArt(line.coffee, height: 88),
                  ),
                  Expanded(
                    child: DVBox.list([
                      DVText(line.coffee.name).modifier(p.headline),
                      DVText('${line.coffee.origin} · ${line.coffee.weightGrams} g')
            ...
            DVBox.list([
              ...
              SummaryLine('Total', formatPrice(subtotal + shipping), strong: true),
              FilledButton(
