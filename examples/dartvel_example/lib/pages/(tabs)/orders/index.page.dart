...
Widget _ordersPage(BuildContext context) => (() {
      ...
      return ShopScroll(children: <Widget>[
        const PageHeading('Orders', subtitle: 'Statuses update as we roast, pack and ship.'),
        WatchModels<Order>(
          watch: Order.watch,
          builder: (BuildContext context, List<Order>? all) {
            ...
            if (mine.isEmpty) {
              return EmptyState(
                ...
                title: 'No orders yet',
                message: 'When you order a coffee, you can follow it from the '
              ...
              for (final Order order in mine)
                Material(
                  ...
                  child: InkWell(
                    ...
                    onTap: () => DV.Navigation.navigate(
                      DVRoutes.ordersId(id: order.id),
                    ),
                    child: DVBox.row([
                      Expanded(
                        child: DVBox.list([
                          DVText('${order.id} · ${shortDate(order.placedAt)}')
                              .modifier(p.overline),
                          DVText(order.summary).modifier(p.headline.maxLines(2)),
                          StatusPill(order.status),
                        ], spacing: 6),
                      ),
                      DVText(formatPrice(order.totalCents)).modifier(p.headline),
