...
|---|---|---|
| **UI Primitives** | `DVBox`, `DVText`, fluent styling through Dartvel's own `DVModifier` chain | ✅ Implemented |
...
- `DV.FileStorage` is one surface on both sides and includes local. On a device it is the file manager — the app's own directories, the pictures and documents a person picks, what it may read and write there. On a server it is that server's filesystem. A bucket on S3, Google Cloud Storage or Azure is one more adapter behind the same calls, never a different API, and local is not a lesser case to be reached some other way.
- **Data models are the only way data is written, and nothing an application sees assumes SQL.** Every create, update and delete in application code, samples, docs, READMEs and examples goes through a generated data model -- `Order(...).save()`, `order.delete()`, `Order.find(...)` -- never through `records.insert/update/delete/find`, `DVRecordAdapter`, `DVRecordTable`, `DVRecord`, a `Map<String, Object?>` record, a database adapter an application constructs, a table or column name, `ensureSchema`, or a SQL string. Dartvel is storage-neutral: SQL, Firestore, MongoDB and DynamoDB sit behind the same models, so an application-facing API that takes a record shape or speaks SQL is wrong for every engine that is not SQL. The record operations in Storage-Neutral Records are the framework's contract with its engines, not a way to write data: they stay out of the barrels an application imports and out of every page that teaches Dartvel. When a feature needs an application-facing hook -- validation, a conflict rule, a capture filter -- it takes the model type, never a map. A sample that has to write a record by hand to make sense means the model surface is missing something; build that, do not document the workaround.
- **A model's capabilities are members of the model, and the machinery behind them is not in the application surface.** Search is `Article.search(...)`, importing is `Article.import(...)`, semantic search is the model's too, and the tuning and the provider go with them. Do not generate a companion class per model — no `ArticleSearch`, no `ArticleImport`, no `<Model>Anything` — and do not document one: a second name for one model's own capability is complexity the reader carries for nothing. The same rule hides what sits underneath. `DVModelSync`, `DVSemanticIndex`, `DVOfflineRemote` and the transports are how the framework delivers, not things an application names, and `DV.Jobs.work(...)` is a worker process draining a queue, not a line in a page. If a sample has to name one of these to make sense, the surface is wrong, not the sample.
- `Model.Form(...)` takes no callback. On the class it is a form that creates a record; on an instance it is a form that edits that record, and saving is what the form does. Policies decide whether the reader may create or edit, as they do everywhere else; a form does not ask the application to wire up its own save.
- Routes are typed everywhere, including in samples and docs. Use the generated `DVRoutes` member; never write `DVRouteTarget('/some/path')` with a literal in application code. A string route drifts the moment the page moves, and the generated target turns that into a compile error. The framework's own internals — a Studio page document whose route is data, a default in the router — are the exception.
- A route with a query is still a typed route. `DVRoutes.signin.withQuery({'from': DVRoutes.account.path})` is two generated targets in one call; `/sign-in?from=/account` written out is two paths in one literal, and neither half is a compile error when either page moves. A module's pages are typed too, through `DV.Modules.<id>Routes`, which the generated barrel exports.
- Every page is reachable by every input method with nothing added: the page shell carries keyboard scrolling, a remote's D-pad and switch control, and they are inert until needed. Never document wrapping a page in `DVSwitchControl` or `DVHardwareKeys`, and never add a widget an application has to remember in order to be accessible. What a reader chooses — which keys their switches send, an auto-scan interval, whether switch control is on — lives on `DV.Accessibility.switchControl`.
- Connectivity is a signal an application reads, never a callback, a plugin or a thing to poll. `DV.Platform.network` has `status` (`online`, `metered`, `offline`, `unknown`), `since`, `changes`, `watch(context)` and `canReachTheServer`, so an offline banner is a widget that rebuilds. `unknown` and `metered` are distinct on purpose: assuming online is how the first write on a train is attempted, fails and is reported as an error rather than queued, and a sync may proceed on a metered connection where a video prefetch should not. Reporting it belongs to the platform binding and is not in the barrel an application imports.
- They are **data models**. Dartvel also has 3D models, and "models" alone reads as either. Say data model in docs, in Studio and in the site's navigation.
- Syncing is a property of a model, not a thing an application wires up. A model opted into syncing is read and written like any other: saving a record publishes the change and watching one receives it. Do not put a transport, a carrier, a hub connection or any other delivery object in the surface an application imports, and do not document one — carrying a change between processes is the framework's own job. `DVModelSyncTransport` and `DVPresenceTransport` exist for the framework and its tests and are hidden from the barrels.
- `DV.Cache` is four calls, `get`, `set`, `has` and `delete`, and everything else is a named option on them: `get(key, compute: ..., ttl: ..., tags: ..., staleFor: ...)` reads through, `set(key, value, tags: ...)` tags, and `delete` takes one positional target as the others take the key: `delete('key')`, `delete(const DVCacheTag('products'))` for every key under a tag, `delete(DVCache.all)` for everything, and anything else is an `ArgumentError`. Do not add another method to it or a `DV.CacheInvalidation`. `dartvel.cache` in `pubspec.yaml` sets the default store and `DV.Cache.withAdapter(adapter)` switches store in code; the lock, housekeeping and tag inspection are the framework's `DVCacheRuntime`, not the application's.
...
- Application-facing Dart declares its classes with primary constructors: `@DVModel() class const _Order({required final String id, final int total = 0});`, `class const Layout({super.key, required super.child}) extends DartvelLayout { ... }`. That covers the `dartvel create` scaffold, examples, docs samples, READMEs, the site and generated models, jobs and widgets. A class that restates each field as a field, a parameter and an initializer is the old shape; do not write new samples in it. The generator reads both shapes, so an existing project keeps building.
...
- Field-scoped model annotations live under the `@DVModel` parent: `@DVModel.sensitiveField()`, `@DVModel.searchableField()`, `@DVModel.featuredImage()`, `@DVModel.pageTitle()`, `@DVModel.mainContent()`, `@DVModel.pageOrder(n)`, `@DVModel.hideFromPage()`, and `@DVModel.model3dField()`. Do not add standalone field annotations such as `@DVFeaturedImage` or `@DVHideFromPage`; group model metadata under the model annotation wherever the grouping is logical.
- Generated model pages are `Model.Page(...)` static members (`.async`, `.signal`, `.fromId`); do not add top-level `ModelPage(...)` wrappers. Every data model gets a public page per record by default, at a route derived from its name (`Article` is `/articles/:slug`, `BlogPost` is `/blog-posts/:id`), and SSG/static paths belong to the model: a model opts out with `@DVModel(generatePublicPages: false)`, and `@DVModel(publicPathsResolver: fn)` names the static paths for a page the application writes itself. Account, session, credential and audit models, rows that are their own privacy subject, tenant-scoped models and models with no `String` key get no page unless they ask with `generatePublicPages: true`. A page shows a `@DVModel.sensitiveField()`, or the field naming the model's privacy `subject:`, only to a viewer the model's `viewSensitive` policy admits, and never puts one in page data, SEO or structured data, static output or the sitemap; a record the model's `view` policy refuses answers 404, never 403, and is not enumerated. Do not add a standalone `@DVStaticPaths` annotation, and never write a route out as a string in an annotation — the model's own route is derived, and a repeated route drifts the moment the page file moves.
...
| `mix` drop-in (migration) | https://github.com/Danroyal001/dartvel_mix | `btwld/mix` | — (a package, not an embedder) |
...
- `mix` is forked as `dartvel_mix` for a different reason than the embedders: it is not a build target but a migration path for projects that already use `mix`. **`DVBox` and `DVText` are not built on `mix`**: they are Dartvel's own `DVModifier` chain rendered with Flutter widgets. `dartvel_flutter` declares pub's `mix` and imports nothing from it, so a `mix` modifier with no `DVModifier` counterpart is not available through the chain. Dartvel's SDK floor is Dartvel's own decision, declared by every package; it is not inherited from `mix`.
