...
- Search belongs to the model: `Article.search(...)`, and the tuning and the provider with it. Do not generate a separate `<Model>Search` class, and do not document one — a second name for one model's own capability is complexity the reader has to carry for nothing.
