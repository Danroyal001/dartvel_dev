...
- `DV.Cache` is four calls, `get`, `set`, `has` and `delete`, and everything else is a named option on them: `get(key, compute: ..., ttl: ..., tags: ..., staleFor: ...)` reads through, `set(key, value, tags: ...)` tags, `delete(tag: ...)` invalidates and `delete(all: true)` clears. Do not add another method to it or a `DV.CacheInvalidation`. `dartvel.cache` in `pubspec.yaml` sets the default store and `DV.Cache.withAdapter(adapter)` switches store in code; the lock, housekeeping and tag inspection are the framework's `DVCacheRuntime`, not the application's.
