...
| **Offline-First Models** | `@DVModel(offline:)`: `Order.save()`, `destroy()` and reads work with no network; the queue goes to the generated backend by itself on reconnect (SQLite on devices, IndexedDB on the web) and sign-out clears it. `encrypt: true` is not applied yet | ⚠️ Partial |
...
| **Module Sources** | `dartvel add <source>` resolves a source into a module. A Dartvel project is mounted directly; a local OpenAPI document or GraphQL schema is generated into a pure Dart module whose calls go over `DV.Http`. Detection names a Swift package, a Maven artifact, a crate, an npm package and the rest, and refuses each by name: those resolvers do not exist | ⚠️ Partial |
| **Native Binding Graph** | Five binding kinds beneath modules — FFI, JNI, web, WASM, Node — with carriers chosen per environment. The set is closed and checked; none of the kinds is built | ⚠️ Partial |
