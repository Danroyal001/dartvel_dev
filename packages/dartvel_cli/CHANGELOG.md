...
- **An Android development build hot reloads over the network.**
  ...
  the QR code `dartvel dev` prints pairs the phone: the phone
  ...
  needs is refused with `DV-DEVCLIENT-002` on both ends. Pairing is always
  on: every `dartvel dev` serves it and prints the code, whether or not a
  device is connected, and there is no flag to turn it on. The local app
  runs as before when there is a device to run it on; with none, or with
  several and no terminal to choose at, `dartvel dev` keeps serving paired
  devices instead of exiting or waiting on a prompt, and neither the local
  app quitting nor a backend that fails to start ends it. The pairing port
  is `--pairing-port`.
...
- **`dartvel dev` prints a QR code a phone can scan.** With the web-server
  ...
  always a QR code of the `dartvel-dev://` pairing link, which
