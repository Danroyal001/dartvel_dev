...
class ClientGenerator {
  static Future<void> generate({
    ...
    final List<String> crashesOff = <String>[
      for (final DVCrashBuildMode mode in DVCrashBuildMode.values)
        if (!crashConfig.enabledIn(mode)) mode.name,
    ];
    if (crashesOff.isNotEmpty) {
      log('dartvel: DV-CRASH-009 crash reporting is disabled for '
          '${crashesOff.join(', ')} builds, as dartvel.crashes declares.');
    }
    ...
    // The generator's version is recorded here and nowhere else, so an
    // upgrade is one line of diff rather than a header change in every file.
    clientFile.writeAsStringSync('''
// GENERATED – do not edit.
// Generated by dartvel $dartvelCliVersion
library dartvel_client;

// The client is what a page imports, so DV here is the application's: the
// Flutter layer's, with pages, navigation and the rest. Core declares a DV
// of its own for the server side, which backend files reach by importing
// dartvel_core; exporting both under one name would be ambiguous in every
// file that imports the client.
export 'package:dartvel_core/dartvel.dart'
    hide DV, DVObservabilityAndLogging;
...
${_studioOn(dv) ? '  // Studio runs on this project\'s server: an installed app reads the pages\n  // it deployed from the backend at launch, and serves the ones meant for it.\n  DVPageStore.fromBackend = true;\n' : ''}  // A dispatched job is useless without its codec and handler, so they are
  ...
  DVWindowSharedStore.defaultAppKeys = () => dvAppKeyStoreFor('$pkgName');
...
''';
    ...
    File(p.join(libClientDir.path, 'modules_data.g.dart')).writeAsStringSync(
      _moduleRegistrationSource(modules: modules),
    );
    File(p.join(libClientDir.path, 'modules.g.dart')).writeAsStringSync(
      _modulesSource(modules: modules),
    );
    ...
    File(p.join(libClientDir.path, 'home_widgets.g.dart')).writeAsStringSync(
      _homeWidgetsSource(homeWidgets),
    );
