...
DVGeneratedModule dvGenerateDescribedApiModule({
  ...
  final String? baseUrl = servers.isEmpty
      ? null
      : (servers.first is Map ? '${(servers.first! as Map)['url'] ?? ''}' : null);
  ...
  final String packageName = _snake(moduleId);
...
class _Parameter {
  ...
  String get identifier => _camel(name);
}
...
String _pubspec({
  ...
  required String title,
    ...
    '''
# GENERATED by dartvel add from an OpenAPI document.
#
...
name: $packageName
description: ${_yamlScalar('$title, as a Dartvel module.')}
publish_to: none
version: 0.0.1
...
String _library({
  ...
  final String className = '${_className(moduleId)}Api';
  final StringBuffer out = StringBuffer()
    ...
    ..writeln()
    ..writeln('/// A call this module made that the API refused.')
    ..writeln('///')
    ..writeln('/// The status and the operation, never the response body: a')
    ..writeln("/// vendor's error text is not this application's to log.")
    ..writeln('class ${_className(moduleId)}Exception implements Exception {')
    ..writeln('  const ${_className(moduleId)}Exception('
        'this.operation, this.status);')
    ..writeln()
    ..writeln('  final String operation;')
    ..writeln('  final int status;')
    ..writeln()
    ..writeln('  @override')
    ..writeln("  String toString() => '\$operation answered \$status.';")
    ..writeln('}')
    ..writeln()
    ..writeln('/// $title.')
    ..writeln('///')
...
void _writeOperation(StringBuffer out, _Operation op, String moduleId) {
  ...
  out
    ...
    ..writeln('      throw ${_className(moduleId)}Exception('
...
void _writeSchema(
  ...
  final String className = _className(name);
...
String _nameFor(String method, String path) {
  ...
  return _camel('$method ${words.join(' ')}');
}
...
String _camel(String text) {
  final List<String> parts = _words(text);
...
String _snake(String text) =>
    _words(text).map((String part) => part.toLowerCase()).join('_');
