...
class BuildCommand extends Command<void> {
  ...
  /// Writes what a Linux development build needs to pair with `dartvel dev`,
  /// or says why it cannot and returns false: the tunnel, and the block in
  /// the runner's CMakeLists.txt that compiles it into Debug builds.
  bool _writeLinuxDevClient(String root, String? target) {
    final DVDevClientManifest manifest;
    try {
      manifest = dvProjectDevClientManifest(root, 'linux');
    } on DVDevClientProjectException catch (error) {
      Logger.log('❌ ${error.message}');
      return false;
    }
    ...
    final String after = dvLinuxRunnerCmakeWithDevClient(before, enabled: true);
    if (!after.contains('dartvel_dev_client.cc')) {
      Logger.log('❌ $dvLinuxRunnerCmakePath has no add_executable(\${BINARY_NAME} '
          '...) this recognises, so the tunnel was not added.');
      return false;
    }
    final Map<String, String> files = <String, String>{
      dvLinuxDevTunnelPath: dvLinuxDevTunnelSource(manifest),
      dvDevelopmentEntrypoint: dvDevelopmentEntrypointSource(
        package: name,
        target: target ?? 'lib/main.dart',
      ),
  ...
  void _stripLinuxDevClient(String root) {
    ...
    if (!cmake.existsSync()) return;
    final String before = cmake.readAsStringSync();
    final String after = dvLinuxRunnerCmakeWithDevClient(before, enabled: false);
    if (after != before) cmake.writeAsStringSync(after);
  }
  ...
  void _writeDeepLinkDocuments(String root, DVDeepLinkConfig? links) {
