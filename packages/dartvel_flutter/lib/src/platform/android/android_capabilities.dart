...
///
/// Reached through JNI, per the native integration rule — never a platform
/// channel. Almost everything here goes through `Context.getSystemService`.
///
/// This used to say the Context comes from `GetApplicationContext()`, "a C
/// function package:jni exports for exactly this purpose". It does not.
/// The declaration is in `dartjni.h` with no definition behind it, so every
/// binding in this list was dead in every real application until an emulator
/// run said "undefined symbol: GetApplicationContext". The Context now comes
/// from the `dev/dartvel/jni/DartvelContext` ContentProvider that
/// `dartvel build android` writes; the header of `android_bindings_jni.dart`
/// carries the full account. Left uncorrected, this sentence would keep
/// sending the next person to a symbol that does not exist.
///
...
///   * **Biometrics** need an `Activity`, not a `Context`: `BiometricPrompt`
///     attaches to one.
