...
* Servers, deployment including store submission, and the admin mount (`infra`, `deploy`, `admin`)
* SEO (`seo`) and PWA (`pwa`)
...
## Connectivity is a signal, and so is the clock

Application code does not branch on connectivity, but it does *show* it — the
offline banner, the queued-writes count, the "last synced" line. That reading
...
is separate from offline because they call for different behaviour: a sync may
proceed on a metered connection and a video prefetch should not.

The clock is the other thing a disconnected device cannot trust. A kiosk that
has been offline for a week, or a phone whose owner changed the date, will
stamp its queued mutations with a time the server disagrees with — and
last-write-wins resolves conflicts by comparing exactly those stamps. The
device records the offset between its clock and the server's at each sync and
stamps mutations with corrected time, so an unsynced clock changes which write
wins. The uncorrected value stays available; it is what the device's own user
sees on screen.
...
## Signed URLs, and the placeholder before the bytes

Two things ride the signing machinery above rather than needing their own.

A **temporary signed URL for any route**, not only for a stored file. An
email-verification link, a password reset, a preview of an unpublished page,
a one-time download: each is a route plus an expiry plus a signature, and the
alternative every application writes is a token model with its own table, its
own cleanup job and its own replay bug.

```dart
final String link = DV.FileStorage.signedRoute(
  DVRoutes.verifyEmail(token: id),
  expires: const Duration(hours: 24),
  once: true,
);
```

`once: true` is the part worth having in the framework: a link that may be
followed a second time is the difference between a reset flow and an account
takeover, and single use needs a record the application would otherwise keep
itself.

A **placeholder before the bytes arrive**. The build already computes each
image's variants; it also records a handful of bytes -- a blurhash, or a tiny
encoded thumbnail -- that the generated widget paints while the real file
loads. It costs nothing at runtime because it is computed at build, and it is
what stops a page reflowing as images land. The client image cache keys on the
same address `DVImageVariants` computes, so a prefetched variant and a rendered
one are one download, not two.

## Documents are previewed, not only stored

A PDF or an office file uploaded to a model field gets the same treatment an
image does: a first-page render as its poster, generated by the same durable
job, so a list of attachments shows what they are. The renderer is an adapter
-- Dartvel bundles no office suite, for the reason it bundles no encoder -- and
a file type with no adapter is stored and listed by name, which is the honest
degradation rather than a broken thumbnail.
...
flows into the privacy manifest that App Store Deployment writes, so the store
...
In development, one surface records what the application just did: each
request with its lifecycle stages and timings, the queries it ran, the jobs it
enqueued, the mail it sent, the cache keys it touched, the AI calls it made.
Opened from `dartvel devtools`, kept in a bounded local store, and never
enabled in a release build — the reason to state that plainly is that a
recorder of every request and query is an excellent way to leak an application
to whoever can reach it.

It is a rendering of what Monitoring and Distributed Tracing already emit, at
the one moment the developer is looking: locally, where a trace backend is not
running and nobody wants to configure one to find out why a page issued forty
queries. The N+1 diagnostic already names the problem; this is where it is
visible without leaving the machine.
...
## Read replicas and the connection pool

A release that doubles the running instances doubles the connections to the
database, and Postgres counts those. Pool size is declared per deployment
target rather than per process, so the number a release opens is a property of
the release plan and not an emergent one:

```yaml
dartvel:
  database:
    pool: { min: 2, max: 10 }
    replicas: [ "postgres://replica-1/app" ]
```

A replica is read-only by declaration and the generated query layer routes to
one, with the rule that makes replicas safe rather than subtly wrong: **a read
inside a transaction, or in the same request as a write to the same model,
goes to the primary.** Replication lag is measured in milliseconds and user
expectations are not — a form that saves and then reads its own record back
off a replica shows the old value, and that bug is unreproducible on a
developer's single-node database.

## The pipeline is generated too

```bash
dartvel ci init            # GitHub Actions, GitLab CI, or the configured provider
```

What it writes is what this specification already requires of a release:
generation, analyze, the test suites, `dartvel generate --check`, the
compatibility gate against the live client histogram, the migration plan, and
the deploy with its strategy. Generated rather than copied from
documentation, because a pipeline pasted once drifts from the framework's
...
Store Deployment already defines: Play internal testing, TestFlight, Firebase
...
An application's own commands are declared the same way, and are typed rather
than shelled out to:

```dart
@DVCommand(description: 'Backfill search vectors for one tenant')
Future<void> _reindex(String tenant, {bool dryRun = false}) async { ... }
```

`dartvel reindex acme --dry-run` then exists, with parsing, `--help` and
argument types generated from the signature. This is where the one-off scripts
in `tool/` go: a script there has no help, no typed arguments, no access to
models or policies without bootstrapping them by hand, and nothing stops two
...
Firebase App Distribution (`firebase-app-distribution`), each declared under
`dartvel.deploy.stores.<store>` in `pubspec.yaml`, beside the rest of the
...
provider is `firebase-hosting`, and a bare `firebase` names neither, so
`--provider firebase` is refused with the two it could mean. Store submission
has no second command, and a store declared anywhere but
`dartvel.deploy.stores` is refused rather than read, naming the `dartvel.deploy.stores` key each store moved to.
The plan is resolved and validated before anything runs,
...
Designed. The track is read from `dartvel.deploy.stores.play.track` today, and none
...
stores deploy through the same command and the same plan-first discipline,
...
| `DV-STORE-005` | the store does not support an option the deploy asked for | `error` |
